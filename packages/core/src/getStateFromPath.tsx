...
export default function getStateFromPath(
  ...
  if (remaining === '/') {
    ...
    const match = configs.find(
      (config) =>
        // make sure that the pattern for this config is empty
        config.pattern === ''
    );
...
function createNormalizedConfigs(
  ...
  if (typeof config === 'string') {
    ...
    configs.push(createConfigItem(key, routeNames, pattern, config));
    ...
    if (typeof config.path === 'string') {
      ...
      configs.push(
        createConfigItem(key, routeNames, pattern, config.path, config.parse)
      );
    }
